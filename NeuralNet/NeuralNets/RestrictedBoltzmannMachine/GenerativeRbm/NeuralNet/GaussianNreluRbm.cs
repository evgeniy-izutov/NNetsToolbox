...
namespace NeuralNet.GenerativeRbm {
	public sealed class GaussianNreluRbm : RestrictedBoltzmannMachine {
		private readonly Normal _normalGenerator;

		public GaussianNreluRbm() : base() {
			_normalGenerator = new Normal {
				RandomSource = new Random()
			};
		}
		
		public GaussianNreluRbm(int visibleStatesCount, int hiddenStatesCount) : base(visibleStatesCount, hiddenStatesCount) {
			_normalGenerator = new Normal {
				RandomSource = new Random()
			};
		}
		...
		public override void VisibleLayerSampling() {
			for (var i = 0; i < visibleStates.Length; i++) {
				visibleStates[i] += ((float) _normalGenerator.Sample());
			}
		}
