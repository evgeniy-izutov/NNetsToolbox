namespace NeuralNet.MultyLayerPerceptron {
	public sealed class MultyLayerPerceptronFactory : INeuralNetFactory {
		...
		public MultyLayerPerceptronFactory(int[] layersStruct, IActivationFunction hiddenActivationFunction, IActivationFunction outputActivationFunction, IMlpWeightGenerator weightGenerator) {
		...
		public INeuralNet CreateNeuralNet() {
			...
			if (layersCount > 1) {
				if (_outputLayerActivationFunction is SoftmaxFunction) {
					outputNeuronBlock = new SoftmaxSimpleNeuronBlock(_layersStruct[layersCount], lastNeuronBlock, new SoftmaxFunction());
				}
				else {
					outputNeuronBlock = new SimpleNeuronBlock(_layersStruct[layersCount], lastNeuronBlock, _outputLayerActivationFunction); 
				}
			}
			else {
				if (_outputLayerActivationFunction is SoftmaxFunction) {
					outputNeuronBlock = new SoftmaxSimpleNeuronBlock(_layersStruct[layersCount], inputLayerSize, new SoftmaxFunction());
				}
				else {
					outputNeuronBlock = new SimpleNeuronBlock(_layersStruct[layersCount], inputLayerSize, _outputLayerActivationFunction); 
				}
			}
